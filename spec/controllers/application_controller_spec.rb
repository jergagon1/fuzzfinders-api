...
RSpec.describe ApplicationController, type: :controller do
  ...
  describe 'current_user' do
    xit 'if valid credentials, it will create current_user' do
    ...
    context "no user" do
      ...
      xit 'does not create current_user if user is logged out' do
        get :index, {email: "blah@blah.com", password_hash: "password"}
        expect(assigns(:current_user)).to be_nil
      end

      xit "responds with 'invalid credentials' if user is logged out" do
        get :index, {email: "blah@blah.com", password_hash: "password"}
        expect(response.body).to eq ("invalid credentials")
      end

      xit "responds with 'invalid credentials' if you don't give any credentials" do
        get :index
        expect(response.body).to eq ("invalid credentials")
      end
    end
